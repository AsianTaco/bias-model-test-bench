...
#                                                                         MAS=MAS)
#                         power_spec_ax.loglog(k_density, bispect_density['bispectrum'], label="density")
#
#                     try:
...
#                                                                    MAS=MAS)
#                         power_spec_ax.loglog(k_truth, bispec_truth['bispectrum'], label='ground truth')
...
#                         power_spec_ax.loglog(k_counts, bispec_counts['bispectrum'], label='predicted')
#
#                         if ground_truth_field_exists:
#                             power_spec_ratio_ax.loglog(k_truth, bispec_counts['bispectrum'] / bispec_truth['bispectrum'],
...
#                                                                            Ntheta, MAS=MAS)
#                         power_spec_ax.loglog(k_benchmark, bispec_benchmark['bispectrum'], label=benchmark_model_name)
#
#                         if ground_truth_field_exists:
#                             power_spec_ratio_ax.loglog(k_truth, bispec_benchmark['bispectrum'] / bispec_truth['bispectrum'],
#                                             label=f'{benchmark_model_name}')
#
#                     except (IndexError, AttributeError, TypeError):
#                         print("No benchmark count field found in BiasModelData. Skipping plots")
#
#                 power_spec_ax.set_xlabel(r"Angle $\theta$")
#                 power_spec_ax.set_ylabel(r"$B(k)$ [$h^{-3}\mathrm{Mpc}^3$]")
#                 power_spec_ax.legend()
...
#                 power_spec_fig.suptitle(f'{bias_model_name} for\n'
#                              f'voxel size {resolution:.2f}$h^{{-1}}\\mathrm{{Mpc}}^3$ and mass bins {mass_lo_hi}')
#                 power_spec_fig.tight_layout(rect=[0, 0.03, 1, 0.95])
#                 power_spec_fig.savefig(f"{dir_path}/{bias_model_name}_res_{res_i}_mass_{mass_bin_i}.png")
#
#                 if ground_truth_field_exists:
#                     power_spec_ratio_ax.set_xlabel(r"Angle $\theta$")
#                     power_spec_ratio_ax.set_ylabel(r"$B(k) / B_{truth}(k)$ ")
#                     power_spec_ratio_ax.axhline(1, linewidth=.5, linestyle='--', color='black')
#                     power_spec_ratio_ax.set_ylim(1e-1, 1e1)
#                     power_spec_ratio_ax.legend()
#                     power_spec_ratio_fig.tight_layout(pad=.1)
#                     power_spec_ratio_fig.savefig(f"{dir_path}/ratios_res_{res_i}_mass_{mass_bin_i}.png")
